...
(function () {
  ...
  if (!Array.isArray(state.priceScanResults)) state.priceScanResults = [];
  ...
  const SPECIAL_PRICE_TARGETS = [9999, 99999];

  function matchesSpecialPrice(value) {
    const num = toNumber(value);
    if (num == null) return null;
    for (const target of SPECIAL_PRICE_TARGETS) {
      if (Math.abs(num - target) < 0.0001) {
        return target;
      }
    }
    return null;
  ...
  function detectSpecialPrice(offer) {
    ...
    for (const path of candidatePaths) {
      const value = readNestedValue(offer, path);
      const matched = matchesSpecialPrice(value);
      if (matched != null) {
        return { price: matched, source: path.join('.') };
      }
    }
    return null;
  }
  ...
  function buildUi() {
    GM_addStyle(`
      ...
      .ggsel-price-controls { margin-top: 8px; }
      .ggsel-row textarea[readonly] { cursor: text; }
    `);
    ...
    panel.innerHTML = `
      ...
      <div class="ggsel-body">
        ...
        <div class="ggsel-row">
          <label>GGSEL ID с ценой 9 999 или 99 999:</label>
          <textarea id="ggsel-price-results" readonly></textarea>
          <div class="ggsel-small">Найдено: <span id="ggsel-price-count" class="ggsel-muted">0</span></div>
          <div class="ggsel-controls ggsel-price-controls">
            <button id="ggsel-scan-prices" class="ggsel-btn">Поиск цен 9999/99999</button>
            <button id="ggsel-copy-prices" class="ggsel-btn">Копировать GGSEL ID</button>
          </div>
        </div>
    ...
    ui = {
      ...
      scanPrices: panel.querySelector('#ggsel-scan-prices'),
      copyPrices: panel.querySelector('#ggsel-copy-prices'),
      priceResults: panel.querySelector('#ggsel-price-results'),
      priceCount: panel.querySelector('#ggsel-price-count'),
    ...
    ui.scanPrices.addEventListener('click', onScanSpecialPrices);
    ui.copyPrices.addEventListener('click', onCopySpecialPrices);
  ...
  let priceScanInProgress = false;

  function getPriceScanIds() {
    if (!Array.isArray(state.priceScanResults)) return [];
    return state.priceScanResults
      .map(entry => normalizeIdKey(entry?.ggselId ?? entry?.ggsel_id ?? entry))
      .filter(Boolean);
  }

  function updatePriceScanUi() {
    if (!ui) return;
    const ids = getPriceScanIds();
    if (ui.priceResults) ui.priceResults.value = ids.join('\n');
    if (ui.priceCount) ui.priceCount.textContent = String(ids.length);
  }
  ...
  async function onScanSpecialPrices() {
    if (priceScanInProgress) {
      log.warn('Поиск цен уже выполняется. Подождите завершения текущего запроса.');
      return;
    }
    priceScanInProgress = true;
    try {
      log.info('Запускаем поиск товаров с ценой 9 999 или 99 999.');
      await ensureOfferCatalog(true);
      const matches = [];
      const seen = new Set();
      for (const [offerId, offer] of offerCatalog.byOfferId.entries()) {
        const specialPrice = detectSpecialPrice(offer);
        if (!specialPrice) continue;
        const ggselId = normalizeIdKey(offer.ggsel_id);
        if (!ggselId) continue;
        if (seen.has(ggselId)) continue;
        const title = extractOfferTitle(offer);
        matches.push({
          ggselId,
          offerId,
          price: specialPrice.price,
          priceSource: specialPrice.source,
          status: offer.catalogStatus ?? offer.status ?? null,
          title
        });
        seen.add(ggselId);
      }
      matches.sort((a, b) => a.ggselId.localeCompare(b.ggselId, undefined, { numeric: true }));
      state.priceScanResults = matches;
      saveState();
      updatePriceScanUi();
      if (!matches.length) {
        log.warn('Товары с ценой 9 999 или 99 999 не найдены.');
      } else {
        const preview = matches
          .slice(0, 5)
          .map(item => `${item.ggselId} (${item.price})${item.title ? ` — ${shortenForLog(item.title, 60)}` : ''}`)
          .join('; ');
        log.info(`Найдено товаров: ${matches.length}. Пример: ${preview}`);
      }
    } catch (error) {
      log.error('Ошибка при поиске товаров с ценой 9 999 или 99 999:', error?.message || error);
    } finally {
      priceScanInProgress = false;
    }
  }

  async function onCopySpecialPrices() {
    const ids = getPriceScanIds();
    if (!ids.length) {
      alert('Нет результатов для копирования. Сначала выполните поиск.');
      return;
    }
    const text = ids.join('\n');
    try {
      ...
      log.info('GGSEL ID с ценами 9 999 / 99 999 скопированы в буфер обмена.');
      alert('GGSEL ID скопированы в буфер обмена.');
      return;
    } catch (error) {
      let copied = false;
      try {
        ...
        log.error('Не удалось скопировать GGSEL ID:', fallbackError?.message || fallbackError);
      }
      if (copied) {
        log.info('GGSEL ID скопированы через резервный метод.');
        alert('GGSEL ID скопированы в буфер обмена.');
      } else {
        log.error('Не удалось скопировать GGSEL ID:', error?.message || error);
        alert('Не удалось скопировать GGSEL ID. Попробуйте вручную.');
      }
    }
  }
